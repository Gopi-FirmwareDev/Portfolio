...
def find_positive_half_cycle_indices(voltages, timeStamps, pos_peaks, pos_drop_peaks, slope_threshold=0.1):
    ...
    for i in range(length-1):
        if find_start_index:
            if voltages[i] == 0 and voltages[i + 1] > 0:
                temp_start_index = i
                find_start_index = False
        else:
            if voltages[i] == 0:
                temp_end_index = i
                temp_indices.append((temp_start_index, temp_end_index))
                find_start_index = True

    # Handle the case where the last positive half cycle doesn't end with a zero
    if not find_start_index and temp_start_index is not None:
        temp_indices.append((temp_start_index, length - 1))

    for start, end in temp_indices:
        positive_voltage = voltages[start:end+1]
        positive_timeStamps = timeStamps[start:end+1]

        # Compute differences and slopes
        slopes = np.diff(positive_voltage) / np.diff(positive_timeStamps)
        ...
        rising_start_indices = [index for index in rising_start_indices if positive_voltage[index] < 0.1]
        falling_end_indices = [index for index in falling_end_indices if positive_voltage[index] < 0.1]

        # Pair the rising start and falling end indices
        for start_index, end_index in zip(rising_start_indices, falling_end_indices):
            real_start_index = start_index + start
            if end_index:
                real_end_index = end_index + start
            else:
                real_end_index = end_index + end

            indices.append((real_start_index, real_end_index))
...
def find_positive_peaks(voltages):
    pos_peaks, _ = find_peaks(voltages)
